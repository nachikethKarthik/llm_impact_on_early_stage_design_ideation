...
with open('D:/research/llm_impact_on_early_stage_design_ideation/on_campus_students/experiment_2/tokenized_df_of_ideas.pkl', 'rb') as f:
...
def plot_clusters(embeddings_2d):
    ...
    # , 'black', 'white', 'gray', 'lightgray','darkgray', 'maroon', 'olive', 'navy', 'purple', 'teal', 'aqua', 'lime', 'fuchsia', 'silver'
    for i in range(len(colors)):
        x = x_embedded[labels==i]
        y = y_embedded[labels==i]
        plt.scatter(x, y, c=colors[i], label='Cluster {}'.format(i+1), alpha=0.6)
    for i, word in enumerate(words_list):
        plt.annotate(word, xy=(x_embedded[i], y_embedded[i]), xytext=(5, 2), textcoords='offset points', ha='right', va='bottom', fontsize=6,alpha = 0.4)
